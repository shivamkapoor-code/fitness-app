...
export function Settings({ state, update, user, updateProfile, signOut, isAdmin, onNavigate }) {
  ...
  async function saveProfile() {
    ...
    const actualAge = ageInput === '' ? null : Number(ageInput)

    if (!displayName) { showToast('Enter a display name', 'warn'); return }
    if (username.length < 2) { showToast('Username must be at least 2 characters', 'warn'); return }
    if (actualAge !== null && (!Number.isInteger(actualAge) || actualAge < 16 || actualAge > 80)) {
  ...
  function adjustAge(delta) {
    const current = ageInput === '' ? 30 : Number(ageInput)
    const next = Math.min(80, Math.max(16, current + delta))
    ...
    <div className="px-4 py-4 pb-20 space-y-4">
      ...
      <div className="glass" style={{ padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Profile</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>Display Name</div>
            <input
              type="text"
          ...
          <div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>Username</div>
            <input
              type="text"
              value={usernameInput}
          ...
          <button onClick={saveProfile} className="btn-primary" style={{ padding: '10px 0', fontSize: 13, cursor: 'pointer' }}>
            Save Profile
          </button>
        </div>
      </div>
